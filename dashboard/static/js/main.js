...
function isIpv6OrIcmpv6(item) {
    if (!item) return false;
    const proto = String(item.protocol || '').toUpperCase();
    if (proto === 'ICMPV6' || proto === 'IPV6') return true;
    const src = String(item.source_ip || item.src_ip || '');
    const dst = String(item.destination_ip || item.dst_ip || '');
    if (src.includes(':') || dst.includes(':')) return true;
    if (Array.isArray(item.source_ips) && item.source_ips.some(ip => String(ip).includes(':'))) return true;
    if (Array.isArray(item.destination_ips) && item.destination_ips.some(ip => String(ip).includes(':'))) return true;
    return false;
}
function endpoint(ip, port, table = false) {
    if (ip == null || ip === '' || String(ip).includes(':')) return '—';
    const s = String(ip);
...
function visibleAlerts(items) {
    const filtered = items.filter(item => !isIpv6OrIcmpv6(item));
    const searchingInfo = state.alertSearch && state.alertSearch.toUpperCase() === 'INFO';
    return (state.showInfo || searchingInfo) ? filtered : filtered.filter(item => !INFORMATIONAL.has(String(item.severity || '').toUpperCase()));
}
...
function severity(value) { const name = String(value || 'LOW').toUpperCase(); return `<span class="badge ${['CRITICAL','HIGH','MEDIUM','LOW','INFO'].includes(name) ? name.toLowerCase() : 'low'}">${esc(name)}</span>`; }
...
function alertHeadings() { return ['ID', 'Last seen', 'Severity', 'Attempts', 'SID', 'Source', 'Destination', 'Protocol', 'Message']; }
function renderSeverity(items) {
    const target = $('severity');
    if (!target) return;
    const cleanItems = items.filter(item => !isIpv6OrIcmpv6(item));
    const levels = ['CRITICAL','HIGH','MEDIUM','LOW','INFO'];
    const counts = levels.map(level => [level, cleanItems.filter(item => String(item.severity || '').toUpperCase() === level).length]);
    const max = Math.max(...counts.map(x => x[1]), 1);
    target.innerHTML = counts.map(([level, count]) => `<div class="severity-row" data-severity="${level}"><span class="severity-label ${level.toLowerCase()}">${level}</span><b>${count}</b><span class="bar ${level.toLowerCase()}"><i style="width:${count / max * 100}%"></i></span></div>`).join('');
    target.querySelectorAll('.severity-row').forEach(row => {
        row.addEventListener('click', () => {
            const level = row.dataset.severity;
            state.alertSearch = level;
            if (level === 'INFO') state.showInfo = true;
            const searchInput = $('alert-search');
            if (searchInput) searchInput.value = level;
            navigate('alerts');
        });
    });
}
...
async function loadTraffic(search = '') { const suffix = search ? `&search=${encodeURIComponent(search)}` : ''; const page = envelope(await api(`/api/traffic?page=1&page_size=500${suffix}`)); const cleanItems = page.items.filter(item => !isIpv6OrIcmpv6(item)); if ($('traffic-table')) $('traffic-table').innerHTML = table(['Time','Source','Destination','Protocol','Length','ID'], cleanItems.map(item => `<tr class="clickable" data-traffic-id="${esc(item.id)}"><td>${time(item.timestamp)}</td><td class="mono">${esc(endpoint(item.src_ip, item.src_port, true))}</td><td class="mono">${esc(endpoint(item.dst_ip, item.dst_port, true))}</td><td>${esc(item.protocol)}</td><td>${number(item.length)}</td><td>${esc(item.id)}</td></tr>`), 'No traffic logs match this search.'); }
async function loadIncidents(search = '') { const suffix = search ? `&search=${encodeURIComponent(search)}` : ''; const page = envelope(await api(`/api/incidents?page=1&page_size=500${suffix}`)); const cleanItems = page.items.filter(item => !isIpv6OrIcmpv6(item)); if ($('incidents-table')) $('incidents-table').innerHTML = table(['ID','First seen','Last seen','Status','Severity','Category','Events','Confidence','Risk','Explanation'], cleanItems.map(item => `<tr class="clickable" data-incident-id="${esc(item.id)}"><td>${esc(item.id)}</td><td>${time(item.first_seen)}</td><td>${time(item.last_seen)}</td><td>${esc(item.status)}</td><td>${severity(item.severity)}</td><td>${esc(item.category)}</td><td>${esc(item.event_count)}</td><td>${esc(item.confidence)}</td><td>${esc(item.risk)}</td><td>${esc(item.explanation)}</td></tr>`), 'No incidents have been recorded.'); }
